...
public class NonmemberChoice_Windowbuilder extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					NonmemberChoice_Windowbuilder frame = new NonmemberChoice_Windowbuilder();
	 ...
	 */
	// 프레임 설정
	public NonmemberChoice_Windowbuilder() {
		setTitle("\uBE44\uD68C\uC6D0 \uB9E4\uC7A5/\uD3EC\uC7A5"); // 최상단 타이틀(비회원 매장/포장)
		...
		String path = "D:\\김동환\\NoljiMaRa\\image\\Maezang.jpg";
		...
		btnMae.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				dispose( ) ;
				setVisible(false) ;
				new NonmemberOder_Shop ( ).setVisible(true) ;
			}
		});
		...
		// 사진경로
		// path1 라는 변수를 선언을 한 후, 사진의 경로를 넣어줌으로써 사진을 변수에 저장.
		String path1= "D:\\김동환\\NoljiMaRa\\image\\Pozang.jpg";
		...
		btnPo.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				dispose( ) ;
				setVisible(false) ;
				new Nonmember_Takeout ( ).setVisible(true) ;
			}
		});
